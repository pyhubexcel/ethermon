...
CATCH_MONSTER_METHOD_ID = "0xb05d6e80"

def update_kyber_wrapper_contract():
	infura_client = InfuraClient(INFURA_API_URL)
	current_block = infura_client.getCurrentBlock() - 2

	start_block = ema_settings_manager.get_setting_value(EmaSetting.SETTING_CONTRACT_KYBER_WRAPPER_BLOCK)
	end_block = start_block + SMART_CONTRACT_BLOCK_STEP

	start_time = time.time()
	transaction_list = get_txn_list(EtheremonKyberContract.ADDRESS, start_block, end_block, ETHERSCAN_API_KEY)
	if transaction_list is None:
		return

	log.data("start_update_kyber_wrapper_contract|current_block=%s,start_block=%s,end_block=%s,len_txn=%s", current_block, start_block, end_block, len(transaction_list))
	...
	for transaction in transaction_list:
		...
		log.info("update_kyber_wrapper|txn_hash=%s,sender=%s", transaction["hash"], sender)
		method_id = transaction["input"][0:SMART_CONTRACT_METHOD_ID_LENGTH]

		if method_id == CATCH_MONSTER_METHOD_ID:

			monster_class_id_hash = "0x" + transaction["input"][SMART_CONTRACT_METHOD_ID_LENGTH + 128:SMART_CONTRACT_METHOD_ID_LENGTH + 192]
			...
			if not trainer_record:
				...
				trainer_record.save()
			log.data("player_kyber_catch_monster|address=%s,class_id=%s", trainer_address, monster_class_id)


	if sender_set:
		log.info("update_kyber_wrapper|len_sender=%s,latest_load_block=%s", len(sender_set), latest_load_block)
	...
	ema_settings_manager.set_setting(EmaSetting.SETTING_CONTRACT_KYBER_WRAPPER_BLOCK, record_block)
	elapsed = time.time() - start_time
	if elapsed < 5:
		time.sleep(5-elapsed)
	log.data("end_update_kyber_contract|record_block=%s,total_txn=%s,total_player=%s,elapsed=%s",
		record_block, len(transaction_list), len(sender_set), elapsed)


if __name__ == "__main__":
	update_kyber_wrapper_contract()
